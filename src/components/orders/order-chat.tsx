...
export function OrderChat({ order: initialOrder, admin = false }: OrderChatProps) {
  ...
  return (
    <section className="order-chat"><div className="order-chat-header"><div><span className="admin-kicker">Order {order.id}</span><h1>{admin ? order.email : "Your order"}</h1></div><span className="order-status">{status.replace("_", " ")}</span></div>
      ...
      {!admin && ["requested", "accepted"].includes(status) && order.paymentStatus === "pending" && <div className="order-payment-callout"><div><strong>Complete your payment.</strong><span>Payment will send your order to the admin delivery queue.</span></div><Button onClick={startPayment} disabled={isPaying}>{isPaying ? "Opening payment..." : "Pay now"}<ArrowRight size={15} /></Button>{paymentError && <small>{paymentError}</small>}</div>}
