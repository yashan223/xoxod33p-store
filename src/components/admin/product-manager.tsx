...
type Draft = { name: string; type: ProductType; description: string; meta: string; price: string; accent: string; imageUrl: string; active: boolean };
const blankDraft: Draft = { name: "", type: "server", description: "", meta: "", price: "", accent: "slate", imageUrl: "", active: true };
...
export function ProductManager({ products: initialProducts }: ProductManagerProps) {
  ...
  function openCreate() { setEditingId(null); setDraft(blankDraft); setStatus(""); setIsEditorOpen(true); }
  function editProduct(product: Product) { setEditingId(product.id); setDraft({ name: product.name, type: product.type, description: product.description, meta: product.meta, price: String(product.price), accent: product.accent, imageUrl: product.imageUrl ?? "", active: product.active }); setStatus(""); setIsEditorOpen(true); }
  function closeEditor() { setIsEditorOpen(false); setEditingId(null); setDraft(blankDraft); setStatus(""); }
  ...
  async function saveProduct(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (editingId) setProducts((current) => current.map((product) => product.id === editingId ? { ...product, ...draft, price: Number(draft.price) } : product));
    else if (result.product) setProducts((current) => [result.product!, ...current]);
    closeEditor();
  }
