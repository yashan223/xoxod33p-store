...
export async function StorePageHeader({ backHref, backLabel = "Back to store" }: StorePageHeaderProps) {
  ...
  return (
    <header className="store-page-header">
      ...
      <div className="store-page-actions">{user ? <Link className="store-page-account" href="/dashboard">Dashboard</Link> : <Link className="store-page-account" href="/sign-in">Sign in</Link>}<Link className="store-page-cart ui-button ui-button-default" href="/#catalog"><ShoppingBag size={17} /> Cart</Link>{backHref && <Link className="back-link" href={backHref}><ArrowLeft size={15} /> {backLabel}</Link>}</div>
    </header>
  );
}
