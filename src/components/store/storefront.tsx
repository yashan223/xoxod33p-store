  ...
  return (
    <main className="store-shell">
      <header className={cn("site-header", isScrolled && "site-header-scrolled")} id="storefront">
        ...
        <div className="header-actions"><Link className="ui-button ui-button-ghost sign-in-button" href="/sign-in">Sign in</Link><Button variant="outline" className="cart-button" onClick={() => setIsCartOpen(true)}><ShoppingBag size={17} /> Cart <span>{cart.length}</span></Button><Button variant="ghost" size="icon" className="menu-button" onClick={() => setIsMenuOpen((open) => !open)} aria-label={isMenuOpen ? "Close menu" : "Open menu"}>{isMenuOpen ? <X size={20} /> : <Menu size={20} />}</Button></div>
      </header>
