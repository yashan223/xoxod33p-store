...
export async function POST(request: Request) {
  ...
  if (!["requested", "accepted"].includes(order.status)) return NextResponse.json({ error: "This order is no longer available for payment." }, { status: 409 });
  ...
  try {
    ...
    const providerError = error instanceof Error ? error.message : "Unable to create the payment checkout.";
    return NextResponse.json({ error: process.env.NODE_ENV === "development" ? providerError : "Unable to create the payment checkout." }, { status: 502 });
  }
}
