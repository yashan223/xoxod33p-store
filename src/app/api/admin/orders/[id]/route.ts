...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (body.status && !["requested", "accepted", "in_progress", "completed", "cancelled"].includes(body.status)) {
    return NextResponse.json({ error: "Invalid order status." }, { status: 400 });
  }
  if (body.status && !(await updateOrderStatus(id, body.status, admin.id))) {
    return NextResponse.json({ error: "Order not found." }, { status: 404 });
  }
  if (body.message?.trim()) await addOrderMessage(id, admin.id, "admin", body.message.trim());
