...
function productInput(body: Record<string, unknown>) {
  ...
  return { type: type as ProductType, name, description, meta, price, accent: typeof body.accent === "string" && body.accent.trim() ? body.accent.trim() : "slate", active: body.active !== false, available: body.available !== false };
}
